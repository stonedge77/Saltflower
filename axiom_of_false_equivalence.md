...
> **Governance of ε.** The threshold ε is derived from *R*'s published decision rule in context *C* and is fixed before any data from the target pair is examined. This forecloses post hoc threshold shopping — the selection of ε after inspecting the pair so as to manufacture, or suppress, a verdict of load-bearing. As an illustration, not a universal setting: for a binary security decision whose system owner publishes a 5 percentage-point change in the false-accept rate as the threshold of materiality, one sets ε = 0.05 in total variation. The number is supplied by the receiver's published rule, not chosen by the analyst.
...
> **Definition 2 (Admissible expansion).** An *admissible expansion* of the observational boundary *S* to *S′* is a finite, lawful increase in sensors, vantage points, or compute budget that does not itself require performing the collapse under test. This bounds the existential quantifier in the Detection Claim: it ranges over lawful, finite increases in access, never over unlimited or unlawful physical access.
...
The four registers constitute a *committed search basis* for a given test, not a claim of exhaustiveness: if a further register is discovered, the claim extends to it, but any individual test must respect the basis pre-registered for that run.

The sampling frame is fixed in advance by binding the class of collapses under test. Let **class 𝒦** be the set of deliberate equivalence operations performed by a source with intent to influence *R*'s action in context *C*, applied to a pre-registered load-bearing distinction (Definition 1). Fixing 𝒦 before data collection prevents post hoc expansion of the sampling frame — the quiet enlargement of "what counts as a relevant collapse" after results are seen. (The class is written 𝒦 to keep it distinct from the context *C* of Definition 1.)

> **Detection Claim.** If a load-bearing distinction *D* (Definition 1) was collapsed at the source by a deliberate equivalence operation, then there exists a finite expansion of the observational boundary *S* to *S′* such that the mutual information *I*(collapse event ; *R_{S′}*) > 0 for some register *R* in {correction-labor, thermodynamic, computational, economic} accessible within *S′*. The claim is falsified by a class of documented collapses (drawn from class 𝒦) for which *I* = 0 under all admissible expansions of *S* (Definition 2).

Here the *collapse event* is the binary indicator that the source asserted *a* = *b* at time *t* for the load-bearing distinction *D*; the mutual information *I*(collapse event ; *R_{S′}*) is therefore the reduction in uncertainty about whether that assertion occurred, given the register *R* observed under expansion *S′*.

The form of the claim is deliberate. It preserves "recoverable in principle" while refusing "detectable in practice everywhere": the existential quantifier over expansions *S′* concedes that detection may require access the observer does not currently possess, and the predicted effect is statistical — for collapses in class 𝒦, a search over the register set recovers a remainder at a rate above baseline. To keep this testable rather than rhetorical, "bounded search" is itself pre-registered as a protocol bundle, fixed before data collection: a compute budget per register, a maximum number of vantage changes, a time window within which the collapse event must fall, and a matched control set of non-collapsed distinctions against which the baseline recovery rate is estimated. The reported quantity is the hit rate on class 𝒦 minus the hit rate on the matched control. Whether the displaced difference is recoverable in a given case depends on the observer's access to the relevant register. The claim is not that every violation is detected, but that every violation in the Lie regime produces a remainder detectable given sufficient access, so that the search is never in principle futile.

Stated in one line for review: *for a pre-registered load-bearing pair (A, B), if a source performs a deliberate equivalence operation at the source, then there exists an admissible finite expansion S′ such that I(collapse ; R_{S′}) > 0 for some register R in {correction-labor, thermodynamic, computational, economic}, with hit rate above baseline (matched control) for collapses in class 𝒦 under the pre-registered search protocol.*
