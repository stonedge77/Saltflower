...
class ToffoliNAND:
    ...
    def _build_decomposed_toffoli(self):
        ...
        # Decomposition in application order
        gates_list = [
        ...
        # Build unitary (reverse order for matrix multiplication)
        U = qt.qeye([2, 2, 2])
    ...
    def verify_equivalence(self):
        ...
        # Find global phase
        nonzero = np.where(np.abs(M_std) > 1e-10)
        ratio = M_decomp[nonzero] / M_std[nonzero]
        phase = np.angle(ratio[0])
        
        # Normalize and compare
        M_std_norm = M_std * np.exp(1j * phase)
        match = np.allclose(M_decomp, M_std_norm, atol=1e-10)
...
def main():
    ...
    # Verify equivalence
    match, phase = toffoli.verify_equivalence()
    print(f"✓ Decomposed Toffoli matches standard: {match}")
